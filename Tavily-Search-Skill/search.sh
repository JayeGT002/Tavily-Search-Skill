...
remaining=$(echo "$quota_response" | jq -r '.account.plan_usage // 0' 2>/dev/null)
total=$(echo "$quota_response" | jq -r '.account.plan_limit // 1000' 2>/dev/null)
used=$(echo "$quota_response" | jq -r '(.account.plan_limit // 1000) - (.account.plan_usage // 0)' 2>/dev/null)
...
if [ -f "$blocklist_file" ] && [ -f "$filter_script" ]; then
    echo "$output" > /tmp/tavily_input.json
    filtered=$(python3 "$filter_script" "$blocklist_file" 2>&1)
    rm -f /tmp/tavily_input.json
    echo "$filtered"
else
    echo "$output"
fi
